...
const MAIN_ID = 'rankings-main'

/**
 * The chrome around the public rankings route.
 *
 * Read by people who are not signed in, so it cannot use `AppShell` — that shell sits inside
 * the console auth guard and owns its own `main` landmark. `features/pricing/PublicFrame`
 * anticipated exactly this ("a shared public shell belongs in `src/components/layout/` once a
 * second public route needs one") and this is that second route; the lane rules forbid this
 * agent from creating the shared component, so the structure is mirrored here and the gap is
 * reported. It borrows only the `.landing-*` classes from the shared `styles/index.css`.
 *
 * The nav deliberately does not link to `/rankings` itself: the route is registered by the
 * integrator, so a typed `to="/rankings"` would not compile until then.
 */
export function RankingsFrame(props: { children: ReactNode }) {
      ...
      <footer className="border-t border-white/10 bg-[#05070a]">
        <div className="mx-auto flex max-w-[1440px] flex-col items-center gap-4 px-4 py-8 text-sm sm:px-8 lg:flex-row lg:justify-between lg:px-12">
          <p className="text-muted">
            {t('Rankings count tokens relayed through this gateway and are refreshed every five minutes.')}
          </p>
